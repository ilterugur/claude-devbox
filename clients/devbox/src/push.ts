...
export function buildMappings(
  sourceCwd: string,
  remoteRoot: string,
  opts: { map?: string[]; remapHome?: boolean; profile: string },
): Mapping[] {
  const base: Mapping[] = [{ from: sourceCwd, to: remoteRoot }];
  ...
  if (opts.remapHome) {
    const u = /^(\/Users\/[^/]+|\/home\/[^/]+)/.exec(sourceCwd);
    if (u) base.push({ from: u[1], to: `/home/${opts.profile}` });
  }
  const all: Mapping[] = [];
  for (const m of base) {
    all.push(m);
...
function changedLines(orig: string, mappings: Mapping[], limit = 8): string[] {
  const out: string[] = [];
 ...
 */
function stageSidecar(srcDir: string, dstDir: string, mappings: Mapping[]): { files: number; bytes: number } {
  let files = 0;
  let bytes = 0;
...
export async function runPush(cfg: Config, opts: PushOpts): Promise<void> {
  ...
  const mappings = buildMappings(sourceCwd, remoteRoot, { map: opts.map, remapHome: opts.remapHome, profile });
